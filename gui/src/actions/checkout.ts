...
import { book, bookLoans, fines } from "@/db/schema";
...
export const checkOut = async (
  ...
  try {
    ...
    const bookAlreadyCheckedOut = await db
      .select()
      .from(bookLoans)
      .where(and(eq(bookLoans.bookIsbn13, isbn13), isNull(bookLoans.dateIn)))
      .limit(1);

    if (bookAlreadyCheckedOut.length > 0) {
      throw new Error(
        `This book is already checked out by another borrower and cannot be checked out at this time.`,
      );
    }
    ...
    if (borrowerFines.length > 0) {
      throw new Error(
        `Cannot check out books. You have unpaid fines that must be paid first.`,
      );
    }
    ...
    if (currentCount >= 3) {
      throw new Error(
        `Cannot check out more books. You already have ${currentCount} book(s) checked out. Maximum is 3.`,
      );
    }
    ...
    const errorMessage =
      err instanceof Error ? err.message : "Failed to check out book";
